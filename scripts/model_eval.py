...
features, feature_cols = fc.get_features(df=raw_df)
features = features.merge(label, how='left', left_index=True, right_index=True)
logging.info(f"Model eval...")
res = pipeline.eval(X=features[feature_cols], y=features['is_clicked'], default_fig_dir=eval_res_dir)
logging.info(f"Eval result saved to {eval_res_dir}...")
