...
class FeatureCreatorV6(BaseFeatureCreator):
    def __init__(self,item_feature_class=None, user_feature_class=None, feature_cols=[]):
    ...
    def _get_normalized_features(self):
        feature_lst = []
        # normalized with historical data
        self.feature_data['normalized_price_history'] = self.feature_data['price_usd']/np.exp(self.feature_data['prop_log_historical_price'])
        feature_lst.append('normalized_price_history')
        # other dimensions
        groupby_col_lst= ['prop_country_id',
                          'srch_booking_window',
                          'srch_length_of_stay',
                          'srch_destination_id',
                          'month'
        ...
        for groupby_col in groupby_col_lst:
            ...
            for col in col_lst:
                percentile_col = '_'.join([col, groupby_col, 'percentile'])
                rank_percentile_col = '_'.join([col, groupby_col, 'rank_percentile'])
                feature_lst.append(percentile_col)
                feature_lst.append(rank_percentile_col)
                self.feature_data[col + '_rank'] = self.feature_data.groupby(groupby_col)[col].rank(method='dense',
                                                                                                    ascending=True)
                self.info = self.feature_data.groupby(groupby_col).agg({col: [np.max, np.min]})[
                    col].reset_index().rename(
                    columns={'amin': col + '_min', 'amax': col + '_max'})
                self.rank_info = self.feature_data.groupby(groupby_col).agg({col + '_rank': [np.max, np.min]})[
                ...
                self.rank_info[col + '_rank_diff'] = self.rank_info[col + '_rank_max'] - self.rank_info[
                    col + '_rank_min']
                ...
                self.rank_info[col + '_rank_diff'] = self.rank_info.apply(lambda row: change_0(row, col + '_rank_diff'),
                                                                          axis=1)
                self.feature_data = self.feature_data.merge(self.info, how='left', on=groupby_col)
                self.feature_data = self.feature_data.merge(self.rank_info, how='left', on=groupby_col)
                self.feature_data[percentile_col] = (self.feature_data[col] - self.feature_data[col + '_min']) / \
                                                    self.feature_data[col + '_diff']
                self.feature_data[rank_percentile_col] = (self.feature_data[col + '_rank'] - self.feature_data[
