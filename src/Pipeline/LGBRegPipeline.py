...
class LGBRegPipeline(BasePipeline):
    def __init__(self, model_path: str, model_training=False, model_params={}, **kwargs):
        super(LGBRegPipeline, self).__init__(model_path=model_path, model_training=model_training)
    ...
    def train(self, X, y, train_params) -> None:
        ...
        self.xgb = LGBMRegressor(**self.model_params)
        if train_valid:
            eval_X = test_X.copy()
            eval_y = test_y.copy()
